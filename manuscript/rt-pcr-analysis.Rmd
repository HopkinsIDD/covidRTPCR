...
pcr_dat3 <- read_csv("data/antibody-test-data.csv") %>% 
    mutate(n_adj=n+nqp,
           test_pos_adj=test_pos+nqp) %>% 
    filter(grepl("RT_PCR", test),
           # !grepl("Danis", study),
...
pcr_dat7 <- read_csv("data/antibody-test-data.csv") %>% 
    mutate(n_adj=n+nqp,
           test_pos_adj=test_pos+nqp) %>% 
    filter(grepl("RT_PCR", test),
           # !grepl("Danis", study),
...
kuj_neg_dat <- read_csv("data/antibody-test-data.csv") %>% 
...
kuj_pos_dat <- read_csv("data/antibody-test-data.csv") %>% 
    mutate(n_adj = n+inconclusive+nqp,
...
```{r raw-figures}
ggplot(data=pcr_dat, aes(x=day, y=pct_pos, size=n_adj, color=study)) +
    facet_grid(.~test) +
